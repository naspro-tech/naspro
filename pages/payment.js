...
export default function PaymentPage() {
  ...
  useEffect(() => {
    if (service && amount) {
      ...
      // Generate payload from backend
      fetch("/api/jazzcash_payment", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          amount: Number(amount),
          description: `Payment for ${SERVICE_LABELS[service] || service} - ${name}`,
          mobileNumber: phone,
          name,
          email,
          service,
        ...
        .then((data) => {
          if (data.success && data.payload) {
            ...
            setApiUrl("https://sandbox.jazzcash.com.pk/CustomerPortal/Transactionmanagement/merchantform/");
          } else {
            alert("Failed to generate JazzCash payment.");
          }
        })
        .catch((err) => {
          console.error(err);
  ...
  const handleJazzCashPayment = (e) => {
    e.preventDefault();
    if (!payload || !apiUrl) {
      alert("Payment is not ready yet.");
      return;
    }
    // Direct form submission - treated as user action
    document.getElementById("jazzcashForm").submit();
  };

  if (!payload) return <p>Loading payment details...</p>;
